...
NEW = {  # instruments to download + segment (pyin range + sane MIDI window to reject octave errors)
    "gayageum": dict(label="가야금 Gayageum", seqs=[2538], fmin="C2", fmax="C6", lo=36, hi=78),
    "daegeum":  dict(label="대금 Daegeum",    seqs=[2557], fmin="A3", fmax="C7", lo=60, hi=96),
    "piri":     dict(label="피리 Piri",       seqs=[2505], fmin="G3", fmax="C7", lo=55, hi=92),
    "haegeum":  dict(label="해금 Haegeum",    seqs=[2482], fmin="C3", fmax="C7", lo=50, hi=96),
}
...
manifest["geomungo"] = dict(label="거문고 Geomungo", midis=geo_midis)
...
for name, cfg in NEW.items():
    ...
    manifest[name] = dict(label=cfg["label"], midis=midis)
